...
const Dashboard = () => {
    ...
    const storedConversation = useMemo(() => {
    ...
    const sendMessage = async(e) => {
        ...
        try{
            ...
            setMessage('');
            await fetchMessages(messages?.conversationId, storedConversation?.user);
        } catch(error){
            console.log(error);
        }
    }
    ...
    useEffect(() => {
        ...
        socket?.on("getMessage", data => {
            ...
            fetchMessages(data.conversationId, data.receiverId);
            // setMessages(prev => ({
            //     ...prev,
        ...
        fetchUsers();
        fetchMessages(storedConversation?.conversationId, storedConversation?.receiverId);
        fetchConversations();
    },[socket])
    ...
    return (
        <div className="flex h-screen w-screen">
            <div className="hidden sm:block sm:w-[30%] lg:w-[25%] bg-slate-200">
                <div className="flex flex-col m-1 lg:flex-row items-center justify-between lg:my-6 lg:mx-4">
                    ...
                    <div className='cursor-pointer' onClick={logout}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#000000" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-9A2.25 2.25 0 0 0 2.25 5.25v13.5A2.25 2.25 0 0 0 4.5 21h9a2.25 2.25 0 0 0 2.25-2.25V15m0-6l3 3m0 0-3 3m3-3H9" />
                        </svg>
                    </div>
                </div>
                        ...
                        conversations.map(({ conversationId, user}) => {
                            return (
                                <div className="flex items-center py-6 border-b border-b-gray-500">
                                    <div className="cursor-pointer flex sm:flex-col lg:flex-row items-center" onClick={() => {setCurrentConversation({conversationId, user}); fetchMessages(conversationId, user)}}>
            ...
            <div className="w-screen sm:w-[70%] lg:w-[50%] flex flex-col items-center">
                    ...
                    <div className="w-full h-[10%] bg-slate-500 flex items-center justify-center py-8">
                        <div className="w-[75%] rounded-full flex items-center p-2">
                            ...
                            <div className="ml-6 mr-auto">
                                <h2 className="text-lg text-white">{storedConversation?.user?.name}</h2>
                                <p className="text-sm text-gray-600 text-white">{storedConversation?.user?.email}</p>
                            </div>
                            <div className="cursor-pointer">
                ...
                { storedConversation?.user?.name &&
                    <div className="w-full h-[10%] flex items-center bg-slate-500 justify-around">
                        <input className="rounded-lg shadow-md w-[75%] py-3 focus:ring-0 outline-none px-4 text-lg max-h-[90%]" type="text" name="message" value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type a message..." required></input>
                        <div className="flex">
                            ...
                            <div className="ml-2 p-2 cursor-pointer rounded-full">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#ffffff" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2f6b3c" class="w-8 h-8">
            ...
            <div className="w-[25%] hidden lg:block bg-slate-200 p-8">
                <div className="text-lg text-teal-700 overflow-screen">People</div>
                {
                        ...
                        users.map(({ userId, user}) => {
                            return (
                                <div className="flex items-center py-6 border-b border-b-gray-500">
                                    <div className="cursor-pointer flex items-center" onClick={() => {setCurrentConversation({conversationId:'new', user}); fetchMessages('new', user)}}>
